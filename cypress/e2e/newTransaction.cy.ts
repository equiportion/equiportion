...
describe('/', () => {
  ...
  it('submit button disabled when creditor missing', () => {
    authenticated(() => {
      ...
      cy.get('#submitButton').should('have.class', 'disabled h-12 w-12 inline-block rounded-full border border-gray-200 bg-gray-400 text-white transition');
    });
  });
  ...
  it('shows error message if fixed amount greater as sum', () => {
    authenticated(() => {
      ...
      cy.get('#unevenSplitting>div').eq(1).children().eq(2).should('contain', 'Die Summe der festen Beträge darf nicht größer als der Gesamtbetrag sein');
      cy.get('#unevenSplitting>div').eq(5).children().eq(2).should('contain', 'Die Summe der festen Beträge darf nicht größer als der Gesamtbetrag sein');
      cy.get('#rest').should('contain', 'Die Summe der festen Beträge darf nicht größer als der Gesamtbetrag sein');
    });
  });
});
